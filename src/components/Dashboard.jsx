...
export default function Dashboard({ username, onLogout, updateUsernameState }) {
    ...
    const fetchMyRooms = async () => { try { const res = await fetch(`https://realtimecode-mr24.onrender.com/api/my-rooms/${username}`); setMyRooms(await res.json()); } catch (err) { } };
    const fetchMyInfo = async () => { try { const res = await fetch(`https://realtimecode-mr24.onrender.com/api/my-info/${username}`); const data = await res.json(); if (data.displayId) setMyDisplayId(data.displayId); } catch (err) { } };
    const fetchFriendsData = async () => { try { setMyFriends(await (await fetch(`https://realtimecode-mr24.onrender.com/api/friends/${username}`)).json()); setFriendRequests(await (await fetch(`https://realtimecode-mr24.onrender.com/api/friend-requests/${username}`)).json()); } catch (err) { } };
    ...
    const handleCreateRoom = async (e) => { e.preventDefault(); if (!newRoomName.trim()) return; const randomRoomId = Math.random().toString(36).substring(2, 9); await fetch('https://realtimecode-mr24.onrender.com/api/create-room', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, roomName: newRoomName.trim(), roomId: randomRoomId }) }); navigate(`/room/${randomRoomId}`); };
    const handleJoinRoom = async (e) => { if (e) e.preventDefault(); if (!roomCode.trim()) return; await fetch('https://realtimecode-mr24.onrender.com/api/join-room', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, roomId: roomCode.trim() }) }); navigate(`/room/${roomCode.trim()}`); };
    const removeRoom = async (roomId) => { if (window.confirm("Bu odayı listenden silmek istediğine emin misin?")) { await fetch('https://realtimecode-mr24.onrender.com/api/remove-room', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, roomId }) }); fetchMyRooms(); } };

    const handleSendFriendRequest = async (e) => { e.preventDefault(); setFriendMsg({ text: 'İstek gönderiliyor...', type: 'info' }); try { const res = await fetch('https://realtimecode-mr24.onrender.com/api/send-friend-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, friendId: friendIdInput }) }); const data = await res.json(); if (res.ok) { setFriendMsg({ text: data.message, type: 'success' }); setFriendIdInput(''); } else setFriendMsg({ text: data.message, type: 'error' }); } catch (err) { setFriendMsg({ text: 'Sunucu hatası!', type: 'error' }); } };
    const acceptRequest = async (senderId) => { await fetch('https://realtimecode-mr24.onrender.com/api/accept-friend-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, senderId }) }); fetchFriendsData(); };
    const rejectRequest = async (senderId) => { await fetch('https://realtimecode-mr24.onrender.com/api/reject-friend-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, senderId }) }); fetchFriendsData(); };
    const removeFriend = async (friendId) => { if (window.confirm("Arkadaşlıktan çıkarmak istediğine emin misin?")) { await fetch('https://realtimecode-mr24.onrender.com/api/remove-friend', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, friendId }) }); fetchFriendsData(); } };

    const handleUsernameChange = async (e) => { e.preventDefault(); setAccMsg({ text: 'İsim güncelleniyor...', type: 'info' }); try { const res = await fetch('https://realtimecode-mr24.onrender.com/api/update-username', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ oldUsername: username, newUsername }) }); const data = await res.json(); if (res.ok) { setAccMsg({ text: data.message, type: 'success' }); updateUsernameState(data.username); setNewUsername(''); } else setAccMsg({ text: data.message, type: 'error' }); } catch (err) { setAccMsg({ text: 'Sunucu hatası!', type: 'error' }); } };
    const handlePasswordChange = async (e) => { e.preventDefault(); if (newPassword !== newPasswordConfirm) return setAccMsg({ text: 'Yeni şifreler eşleşmiyor!', type: 'error' }); setAccMsg({ text: 'Şifre güncelleniyor...', type: 'info' }); try { const res = await fetch('https://realtimecode-mr24.onrender.com/api/update-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, oldPassword, newPassword }) }); const data = await res.json(); if (res.ok) { setAccMsg({ text: data.message, type: 'success' }); setOldPassword(''); setNewPassword(''); setNewPasswordConfirm(''); } else setAccMsg({ text: data.message, type: 'error' }); } catch (err) { setAccMsg({ text: 'Sunucu hatası!', type: 'error' }); } };
